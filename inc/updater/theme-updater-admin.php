...
class EDD_Theme_Updater_Admin {
	...
	function check_license() {
		...
		if ( isset( $license_data->expires ) ) {
			$expires    = ( $license_data->expires == 'lifetime' ) ? 'never' : date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires ) );
		...
		if ( $license_data->license == 'valid' ) {
			...
			if ( $expires ) {
				$message .= ( $expires == 'never' ) ? $strings['expires-never'] : sprintf( $strings['expires%s'], $expires ) . ' ';
			}
